...
flowchart TB
    subgraph Client Layer
        Client[Client / k6 Load Test]
    end

    subgraph API["Spring Boot App — :8888"]
        Controller[NotificationController]
        Producer[NotificationProducer]
        Scheduler[RateLimitRecoveryScheduler]
    end

    subgraph Storage
        PG[(PostgreSQL<br/>notifications · user_preference)]
        Redis[(Redis<br/>rate_limit:user:{id})]
    end

    subgraph Messaging["Apache Kafka — :9092"]
        MainTopic[notification-events]
        Retry0[notification-events-retry-0]
        Retry1[notification-events-retry-1]
        Retry2[notification-events-retry-2]
        DLQ[notification-events-dlq]
    end

    subgraph Workers["NotificationConsumer — 3 consumer groups"]
        EmailWorker[EMAIL Worker<br/>retry · DLQ · idempotency · rate limit]
    ...
    MainTopic --> EmailWorker
    MainTopic --> SmsWorker
    MainTopic --> PushWorker

    EmailWorker --> PG
    EmailWorker --> Redis
    EmailWorker -.->|failure| Retry0
    Retry0 -.-> Retry1
    Retry1 -.-> Retry2
    Retry2 -.->|exhausted| DLQ

    Scheduler --> PG
    Scheduler --> Producer

    Controller -->|/actuator/prometheus| Prom
    Prom --> Grafana
```
